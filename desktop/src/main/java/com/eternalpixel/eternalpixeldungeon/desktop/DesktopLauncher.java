...
package com.eternalpixel.eternalpixeldungeon.desktop;
...
import com.eternalpixel.eternalpixeldungeon.services.news.NewsImpl;
import com.eternalpixel.eternalpixeldungeon.services.updates.UpdateImpl;
	...
	public static void main (String[] args) {
		...
		Thread.setDefaultUncaughtExceptionHandler(new Thread.UncaughtExceptionHandler() {
			...
			public void uncaughtException(Thread thread, Throwable throwable) {
				...
				exceptionMsg = exceptionMsg.replace("com.eternalpixel.eternalpixeldungeon.", "");
		...
		FileHandle newPrefs = new Lwjgl3FileHandle(basePath + EPDSettings.DEFAULT_PREFS_FILE, Files.FileType.External);
		if (oldPrefs.exists() && !newPrefs.exists()){
			oldPrefs.copyTo(newPrefs);
		}

		config.setPreferencesConfig( basePath, Files.FileType.External );
		EPDSettings.set( new Lwjgl3Preferences( EPDSettings.DEFAULT_PREFS_FILE, basePath) );
		FileUtils.setDefaultFileProperties( Files.FileType.External, basePath );
		
		config.setWindowSizeLimits( 480, 320, -1, -1 );
		Point p = EPDSettings.windowResolution();
		config.setWindowedMode( p.x, p.y );

		config.setMaximized(EPDSettings.windowMaximized());

		if (EPDSettings.fullscreen()) {
		...
		new Lwjgl3Application(new EternalPixelDungeon(new DesktopPlatformSupport()), config);
	}
}

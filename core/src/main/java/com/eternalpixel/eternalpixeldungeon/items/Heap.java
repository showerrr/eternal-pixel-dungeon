...
import com.eternalpixel.eternalpixeldungeon.items.food.FrozenCarpaccio;
import com.eternalpixel.eternalpixeldungeon.items.food.MysteryMeat;
import com.eternalpixel.eternalpixeldungeon.items.journal.DocumentPage;
...
public class Heap implements Bundlable {
	...
	public void drop( Item item ) {
		...
		if (item.stackable && type != Type.FOR_SALE && type != Type.INTERACTIVE) {
		...
		if ((item.dropsDownHeap && type != Type.FOR_SALE && type != Type.INTERACTIVE) || peek() instanceof LostBackpack) {
			items.add( item );
		} else {
			items.addFirst( item );
		}
	...
	public boolean canInteract(Char c){
		if (c instanceof Hero && type == Type.INTERACTIVE && pos == c.pos && items.size() == 1){
			return true;
		} else {
			return false;
		}

...
import com.eternalpixel.eternalpixeldungeon.messages.Languages;
...
import com.eternalpixel.eternalpixeldungeon.services.news.News;
import com.eternalpixel.eternalpixeldungeon.services.updates.Updates;
import com.eternalpixel.eternalpixeldungeon.sprites.CharSprite;
import com.eternalpixel.eternalpixeldungeon.ui.CheckBox;
import com.eternalpixel.eternalpixeldungeon.ui.GameLog;
import com.eternalpixel.eternalpixeldungeon.ui.Icons;
import com.eternalpixel.eternalpixeldungeon.ui.OptionSlider;
...
public class WndSettings extends WndTabbed {
	...
	public void hide() {
		...
		EternalPixelDungeon.seamlessResetScene(new Game.SceneChangeCallback() {
			@Override
		...
		protected void createChildren() {
			...
			chkFullscreen = new CheckBox( Messages.get(this, "fullscreen") ) {
				...
				protected void onClick() {
					super.onClick();
					EPDSettings.fullscreen(checked());
				}
			};
			if (DeviceCompat.supportsFullScreen()){
				chkFullscreen.checked(EPDSettings.fullscreen());
			} else {
			...
			if ((int)Math.ceil(2* Game.density) < PixelScene.maxDefaultZoom) {
				optScale = new OptionSlider(Messages.get(this, "scale"),
					...
					protected void onChange() {
						if (getSelectedValue() != EPDSettings.scale()) {
							EPDSettings.scale(getSelectedValue());
							EternalPixelDungeon.seamlessResetScene();
						}
					}
				};
			...
			if (DeviceCompat.isAndroid() && PixelScene.maxScreenZoom >= 2) {
				chkSaver = new CheckBox(Messages.get(this, "saver")) {
					...
					protected void onClick() {
						...
						if (checked()) {
							checked(!checked());
							EternalPixelDungeon.scene().add(new WndOptions(Icons.get(Icons.DISPLAY),
								...
								protected void onSelect(int index) {
									if (index == 0) {
										checked(!checked());
										EPDSettings.powerSaver(checked());
									}
								}
							});
						} else {
				...
				chkSaver.checked( EPDSettings.powerSaver() );
				add( chkSaver );
			}
			...
			if (DeviceCompat.isAndroid()) {
				btnOrientation = new RedButton(PixelScene.landscape() ?
					...
					@Override
					protected void onClick() {
						EPDSettings.landscape(!PixelScene.landscape());
					}
				};
				add(btnOrientation);
			}
			...
			optBrightness = new OptionSlider(Messages.get(this, "brightness"),
				...
				@Override
				protected void onChange() {
					EPDSettings.brightness(getSelectedValue());
				}
			};
			optBrightness.setSelectedValue(EPDSettings.brightness());
			add(optBrightness);
			...
			optVisGrid = new OptionSlider(Messages.get(this, "visual_grid"),
				...
				@Override
				protected void onChange() {
					EPDSettings.visualGrid(getSelectedValue());
				}
			};
			optVisGrid.setSelectedValue(EPDSettings.visualGrid());
			add(optVisGrid);
		...
		protected void createChildren() {
			...
			btnSplit = new RedButton(Messages.get(this, "split")){
				...
				protected void onClick() {
					...
					EPDSettings.toolbarMode(Toolbar.Mode.SPLIT.name());
					Toolbar.updateLayout();
				}
			};
			if (EPDSettings.toolbarMode().equals(Toolbar.Mode.SPLIT.name())) btnSplit.textColor(TITLE_COLOR);
			add(btnSplit);
			...
			btnGrouped = new RedButton(Messages.get(this, "group")){
				...
				protected void onClick() {
					...
					textColor(TITLE_COLOR);
					btnCentered.textColor(WHITE);
					EPDSettings.toolbarMode(Toolbar.Mode.GROUP.name());
					Toolbar.updateLayout();
				}
			};
			if (EPDSettings.toolbarMode().equals(Toolbar.Mode.GROUP.name())) btnGrouped.textColor(TITLE_COLOR);
			add(btnGrouped);
			...
			btnCentered = new RedButton(Messages.get(this, "center")){
				...
				protected void onClick() {
					...
					btnGrouped.textColor(WHITE);
					textColor(TITLE_COLOR);
					EPDSettings.toolbarMode(Toolbar.Mode.CENTER.name());
					Toolbar.updateLayout();
				}
			};
			if (EPDSettings.toolbarMode().equals(Toolbar.Mode.CENTER.name())) btnCentered.textColor(TITLE_COLOR);
			add(btnCentered);
			...
			chkFlipToolbar = new CheckBox(Messages.get(this, "flip_toolbar")){
				...
				protected void onClick() {
					super.onClick();
					EPDSettings.flipToolbar(checked());
					Toolbar.updateLayout();
				}
			};
			chkFlipToolbar.checked(EPDSettings.flipToolbar());
			add(chkFlipToolbar);
			...
			chkFlipTags = new CheckBox(Messages.get(this, "flip_indicators")){
				...
				protected void onClick() {
					super.onClick();
					EPDSettings.flipTags(checked());
					GameScene.layoutTags();
				}
			};
			chkFlipTags.checked(EPDSettings.flipTags());
			add(chkFlipTags);
			...
			chkFont = new CheckBox(Messages.get(this, "system_font")){
				...
				protected void onClick() {
					...
					EternalPixelDungeon.seamlessResetScene(new Game.SceneChangeCallback() {
						@Override
						public void beforeCreate() {
							EPDSettings.systemFont(checked());
						}
			...
			chkFont.checked(EPDSettings.systemFont());
			add(chkFont);
			...
			if (DeviceCompat.hasHardKeyboard()){
				...
				btnKeyBindings = new RedButton(Messages.get(this, "key_bindings")){
					...
					protected void onClick() {
						super.onClick();
						EternalPixelDungeon.scene().addToFront(new WndKeyBindings());
					}
				};
	...
	private static class DataTab extends Component{
		...
		protected void createChildren() {
			...
			chkNews = new CheckBox(Messages.get(this, "news")){
				...
				protected void onClick() {
					super.onClick();
					EPDSettings.news(checked());
					News.clearArticles();
				}
			};
			chkNews.checked(EPDSettings.news());
			add(chkNews);
			...
			if (Updates.supportsUpdates() && Updates.isUpdateable()) {
				chkUpdates = new CheckBox(Messages.get(this, "updates")) {
					...
					protected void onClick() {
						super.onClick();
						EPDSettings.updates(checked());
						Updates.clearUpdate();
					}
				};
				chkUpdates.checked(EPDSettings.updates());
				add(chkUpdates);
				...
				if (Updates.supportsBetaChannel()){
					...
					chkBetas.checked(EPDSettings.betas());
					add(chkBetas);
				}
			}
			...
			if (!DeviceCompat.isDesktop()){
				chkWifi = new CheckBox(Messages.get(this, "wifi")){
					...
					protected void onClick() {
						super.onClick();
						EPDSettings.WiFi(checked());
					}
				};
				chkWifi.checked(EPDSettings.WiFi());
				add(chkWifi);
			}
		}
	...
	private static class AudioTab extends Component {
		...
		protected void createChildren() {
			...
			optMusic = new OptionSlider(Messages.get(this, "music_vol"), "0", "10", 0, 10) {
				@Override
				protected void onChange() {
					EPDSettings.musicVol(getSelectedValue());
				}
			};
			optMusic.setSelectedValue(EPDSettings.musicVol());
			add(optMusic);
			...
			chkMusicMute = new CheckBox(Messages.get(this, "music_mute")){
				...
				protected void onClick() {
					super.onClick();
					EPDSettings.music(!checked());
				}
			};
			chkMusicMute.checked(!EPDSettings.music());
			add(chkMusicMute);
			...
			optSFX = new OptionSlider(Messages.get(this, "sfx_vol"), "0", "10", 0, 10) {
				@Override
				protected void onChange() {
					EPDSettings.SFXVol(getSelectedValue());
					if (Random.Int(100) == 0){
			...
			optSFX.setSelectedValue(EPDSettings.SFXVol());
			add(optSFX);
			...
			chkMuteSFX = new CheckBox( Messages.get(this, "sfx_mute") ) {
				...
				protected void onClick() {
					super.onClick();
					EPDSettings.soundFx(!checked());
					Sample.INSTANCE.play( Assets.Sounds.CLICK );
				}
			};
			chkMuteSFX.checked(!EPDSettings.soundFx());
			add( chkMuteSFX );
			...
			if (DeviceCompat.isiOS() && Messages.lang() == Languages.ENGLISH){
				...
				chkIgnoreSilent = new CheckBox( "Ignore Silent Mode" ){
					...
					protected void onClick() {
						super.onClick();
						EPDSettings.ignoreSilentMode(checked());
					}
				};
				chkIgnoreSilent.checked(EPDSettings.ignoreSilentMode());
				add(chkIgnoreSilent);
			}
		}
				...
				RedButton btn = new RedButton(Messages.titleCase(langs.get(i).nativeName()), 8){
					...
					protected void onClick() {
						...
						EternalPixelDungeon.seamlessResetScene(new Game.SceneChangeCallback() {
							@Override
							public void beforeCreate() {
								EPDSettings.language(langs.get(langIndex));
								GameLog.wipe();
								Game.platform.resetGenerators();
							}
							@Override
			...
			if (currLang != Languages.ENGLISH) {
				...
				btnCredits = new RedButton(credText, credText.length() > 9 ? 6 : 9) {
					...
					protected void onClick() {
						...
						EternalPixelDungeon.scene().addToFront(credits);
					}
				};
				add(btnCredits);
			}

...
public class WndBag extends WndTabbed {
	...
	protected static final int COLS_L   = 14;
	...
	public boolean onSignal(KeyEvent event) {
		if (event.pressed && KeyBindings.getActionForKey( event ) == EPDAction.INVENTORY) {
			hide();
			return true;
		} else {
	...
	private Image icon( Bag bag ) {
		...
		} else if (bag instanceof CoolerBox) {
			return Icons.get(Icons.FOOD_BOX);
		} else {

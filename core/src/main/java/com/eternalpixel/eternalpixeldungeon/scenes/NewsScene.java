...
import com.eternalpixel.eternalpixeldungeon.services.news.NewsArticle;
...
public class NewsScene extends PixelScene {
	...
	private static class NewsInfo extends Component {
		...
		protected void createChildren() {
			...
			if (!News.articlesAvailable()){
				if (EPDSettings.news()) {
					if (EPDSettings.WiFi() && !Game.platform.connectedToUnmeteredNetwork()) {
	...
	private static class ArticleButton extends StyledButton {
		...
		protected void onClick() {
			...
			if (article.date.getTime() > EPDSettings.newsLastRead()){
				EPDSettings.newsLastRead(article.date.getTime());
			}
			EternalPixelDungeon.scene().addToFront(new WndArticle(article));
		}
	}

...
import com.eternalpixel.eternalpixeldungeon.ui.IconButton;
...
import com.eternalpixel.eternalpixeldungeon.windows.WndChallenges;
import com.eternalpixel.eternalpixeldungeon.windows.WndHeroInfo;
...
public class HeroSelectScene extends PixelScene {
	...
	public void create() {
		...
		startBtn = new StyledButton(Chrome.Type.GREY_BUTTON_TR, ""){
			...
			protected void onClick() {
				...
				if (EPDSettings.intro()) {
					EPDSettings.intro( false );
		...
		infoButton = new IconButton(Icons.get(Icons.INFO)){
			...
			protected void onClick() {
				super.onClick();
				EternalPixelDungeon.scene().addToFront(new WndHeroInfo(GamesInProgress.selectedClass));
			}
		};
		...
		challengeButton = new IconButton(
				Icons.get( EPDSettings.challenges() > 0 ? Icons.CHALLENGE_ON :Icons.CHALLENGE_OFF)){
			@Override
			protected void onClick() {
				EternalPixelDungeon.scene().addToFront(new WndChallenges(EPDSettings.challenges(), true) {
					public void onBackPressed() {
						super.onBackPressed();
						icon(Icons.get(EPDSettings.challenges() > 0 ? Icons.CHALLENGE_ON : Icons.CHALLENGE_OFF));
					}
				} );
			}
		...
		if (DeviceCompat.isDebug() || Badges.isUnlocked(Badges.Badge.VICTORY)){
			...
			EPDSettings.challenges(0);
		}

		btnExit = new ExitButton();
		btnExit.setPos( Camera.main.width - btnExit.width(), 0 );
		add( btnExit );
		btnExit.visible = !EPDSettings.intro() || Rankings.INSTANCE.totalNumber > 0;
	...
	private class HeroBtn extends StyledButton {
		...
		protected void onClick() {
			...
			if( !cl.isUnlocked() ){
				EternalPixelDungeon.scene().addToFront( new WndMessage(cl.unlockMsg()));
			} else if (GamesInProgress.selectedClass == cl) {
				EternalPixelDungeon.scene().add(new WndHeroInfo(cl));
			} else {
				setSelectedHero(cl);
			}
		}

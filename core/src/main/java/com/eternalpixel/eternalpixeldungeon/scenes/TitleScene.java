...
import com.eternalpixel.eternalpixeldungeon.services.updates.AvailableUpdateData;
...
import com.eternalpixel.eternalpixeldungeon.windows.WndSettings;
...
public class TitleScene extends PixelScene {
	...
	public void create() {
		...
		StyledButton btnPlay = new StyledButton(GREY_TR, Messages.get(this, "enter")){
			...
			protected void onClick() {
				if (GamesInProgress.checkAll().size() == 0){
					...
					EternalPixelDungeon.switchNoFade( StartScene.class );
				}
			}
		...
		StyledButton btnRankings = new StyledButton(GREY_TR,Messages.get(this, "rankings")){
			@Override
			protected void onClick() {
				EternalPixelDungeon.switchNoFade( RankingsScene.class );
			}
		};
		...
		StyledButton btnBadges = new StyledButton(GREY_TR, Messages.get(this, "badges")){
			@Override
			protected void onClick() {
				EternalPixelDungeon.switchNoFade( BadgesScene.class );
			}
		};
		...
		StyledButton btnAbout = new StyledButton(GREY_TR, Messages.get(this, "about")){
			@Override
			protected void onClick() {
				EternalPixelDungeon.switchScene( AboutScene.class );
			}
		};
		...
		if (landscape()) {
			...
			align(btnPlay);
			btnRankings.setRect(btnPlay.right()+2, btnPlay.top(), btnPlay.width(), BTN_HEIGHT);
			btnBadges.setRect(btnPlay.left(), btnPlay.bottom()+ GAP, btnPlay.width(), BTN_HEIGHT);
			btnChanges.setRect(btnBadges.left(), btnBadges.bottom()+GAP, btnPlay.width(), BTN_HEIGHT);
			btnSettings.setRect(btnBadges.right()+2, btnBadges.top(), btnPlay.width(), BTN_HEIGHT);
			...
			btnPlay.setRect(title.x, topRegion+GAP, title.width()/2, BTN_HEIGHT);
			align(btnPlay);
			btnRankings.setRect(btnPlay.right()+2, btnPlay.top(), btnPlay.width(), BTN_HEIGHT);
			btnBadges.setRect(btnPlay.left(), btnPlay.bottom()+ GAP, btnPlay.width(), BTN_HEIGHT);
			btnChanges.setRect(btnBadges.left(), btnBadges.bottom()+GAP, btnPlay.width(), BTN_HEIGHT);
	...
	private static class NewsButton extends StyledButton {
		...
		public NewsButton(Chrome.Type type, String label ){
			super(type, label);
			if (EPDSettings.news()) News.checkForNews();
		}
		...
		public void update() {
			...
			if (unreadCount == -1 && News.articlesAvailable()){
				long lastRead = EPDSettings.newsLastRead();
				if (lastRead == 0){
					if (News.articles().get(0) != null) {
						EPDSettings.newsLastRead(News.articles().get(0).date.getTime());
					}
				} else {
					unreadCount = News.unreadArticles(new Date(EPDSettings.newsLastRead()));
					if (unreadCount > 0) {
		...
		protected void onClick() {
			super.onClick();
			EternalPixelDungeon.switchNoFade( NewsScene.class );
		}
	}
	...
	private static class ChangesButton extends StyledButton {
		...
		public ChangesButton( Chrome.Type type, String label ){
			super(type, label);
			if (EPDSettings.updates()) Updates.checkForUpdate();
		}
		...
		protected void onClick() {
			if (Updates.isInstallable()){
				...
				EternalPixelDungeon.scene().addToFront(new WndOptions(
					...
					protected void onSelect(int index) {
						if (index == 0) {
							...
							EternalPixelDungeon.switchNoFade( ChangesScene.class );
						}
					}
				});
	...
	private static class SettingsButton extends StyledButton {
		...
		protected void onClick() {
			...
			EternalPixelDungeon.scene().add(new WndSettings());
		}
	}
	...
	private static class SupportButton extends StyledButton{
		...
		@Override
		protected void onClick() {
			EternalPixelDungeon.switchNoFade(SupporterScene.class);
		}
	}
}

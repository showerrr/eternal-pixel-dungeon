...
import com.eternalpixel.eternalpixeldungeon.actors.buffs.Corruption;
...
import com.eternalpixel.eternalpixeldungeon.actors.buffs.SoulMark;
...
import com.eternalpixel.eternalpixeldungeon.actors.mobs.npcs.DirectableAlly;
import com.eternalpixel.eternalpixeldungeon.effects.CellEmitter;
import com.eternalpixel.eternalpixeldungeon.effects.Speck;
import com.eternalpixel.eternalpixeldungeon.effects.Surprise;
import com.eternalpixel.eternalpixeldungeon.effects.Wound;
...
public abstract class Mob extends Char {
	...
	public void die( Object cause ) {
		...
		if (alignment == Alignment.ENEMY){
			...
			RiggedTarot.rogueCard a = Dungeon.hero.buff(RiggedTarot.rogueCard.class);
			if (cause == Dungeon.hero && a != null) {
				a.prolong();
			}
		}
	...
	public void rollToDropLoot(){
		...
		if (Random.Float() < lootChance) {
			...
			if (loot != null) {
				if (loot instanceof MysteryMeat) {
					loot = new Corpse().madeOf(this.name());
				}
				Dungeon.level.drop(loot, pos).sprite.drop();
			}
		} else if (Random.Float() < 0.1) {
			Dungeon.level.drop(new Corpse().madeOf(this.name()), pos).sprite.drop();
		}

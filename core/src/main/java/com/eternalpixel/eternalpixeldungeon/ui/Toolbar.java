...
import com.eternalpixel.eternalpixeldungeon.windows.WndQuickBag;
...
public class Toolbar extends Component {
	...
	protected void layout() {
		...
		for(int i = 0; i <= 3; i++) {
			if (i == 0 && !EPDSettings.flipToolbar() ||
				i == 3 && EPDSettings.flipToolbar()){
				btnQuick[i].border(0, 2);
				btnQuick[i].frame(106, 0, 19, 24);
			} else if (i == 0 && EPDSettings.flipToolbar() ||
					i == 3 && !EPDSettings.flipToolbar()){
		...
		float right = width;
		switch(Mode.valueOf(EPDSettings.toolbarMode())){
			case SPLIT:
				btnWait.setPos(x, y);
